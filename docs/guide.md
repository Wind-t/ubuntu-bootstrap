...
mise 是一个「多语言版本管理器」——它负责下载、安装、管理下面这 19 个工具的精确版本。

| 工具            | 分类   | 你用它干什么                                                                       |
|-----------------|--------|------------------------------------------------------------------------------------|
| `node 24`       | 运行时 | JavaScript/TypeScript 项目。LTS 版本，npm 自带                                     |
| `go 1.26`       | 运行时 | Go 项目。gofmt、go build 全套                                                      |
...
| `editor = micro` | `git commit`（不加 -m）时打开 micro 编辑器 |
...
| 变量 | 值 | 作用 |
|------|-----|------|
| `EDITOR` | `micro` | Git commit、crontab 等默认用 micro 编辑器 |
